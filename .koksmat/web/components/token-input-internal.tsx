  ...
  const selectSuggestion = (suggestion: Suggestion) => {
    if ("value" in suggestion) {
      ...
      if (activeProperty && propertyTokenIndex !== null) {
        ...
        newTokens[propertyTokenIndex] = {
          ...
          start: newTokens[propertyTokenIndex].start,
            ...
            newTokens[propertyTokenIndex].start +
        ...
        setSuggestions([]);
        setPropertyTokenIndex(null);
        // Move cursor to the end of the replaced token
        setTimeout(() => {
      ...
      if (currentTokenIndex !== null) {
        // Replace the current token with the selected property name and colon
        ...
        newTokens.push({
          ...
          end: cursorPosition + suggestion.name.length + 1, // +1 for colon
        });
      }
      // Reconstruct the input value
      const newValue = reconstructInputValue(newTokens);
        ...
        <div className="flex flex-wrap">
          {tokens.map((token, index) => {
            if (token.value.trim() === "") {
                ...
                return (
                  <React.Fragment key={index}>
                    ...
                    <span
                      ...
                      className={isError ? "underline text-red-500" : ""}
                    >
                      {propertyValue}
                    </span>
                  </React.Fragment>
              ...
              // Non-property token or property name
              const isKnownProperty = properties.some(
                (prop) => prop.name === token.value
              );
              return (
                <span
                  key={index}
                  className={isKnownProperty ? "font-bold" : ""}
                >
                  {token.value}
                </span>
              );
